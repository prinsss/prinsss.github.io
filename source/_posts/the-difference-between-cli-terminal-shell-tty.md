...
![dele_cli](https://img.blessing.studio/images/2018/08/22/dele_cli.jpg)
...
大家都知道，操作系统有一个叫做 **内核** (Kernel) 的东西，它管理着整台计算机的硬件，是现代操作系统中最基本的部分。但是，内核处于操作系统的底层，其本身并不提供任何的用户交互界面。

那我们要如何去与内核交互呢？这就需要一个专门的程序，它接受用户输入的命令，解析后交给内核执行，并将执行结果返回给用户。这个提供用户界面的程序被叫做 **Shell** (壳层)。

为什么叫做 Shell？因为它隐藏了操作系统底层的细节实现（Linux 内核现在就有几百个系统调用），像贝壳一样把内核包裹起来，为用户提供了一个更加友好易用的操作界面。

![shell](https://img.blessing.studio/images/2018/08/22/shell.png)
